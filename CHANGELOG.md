...
### 安全修复
- **[高1] 路径穿越**（`fd918a7`）：`static_fallback` 直接拼接路径未验证边界，
  `GET /..%2Fserver.py` 可读源码。修复：resolve 后用 `relative_to(STATIC_DIR)` 验证。
- **[高2] 远程脚本**（`551dea0`）：删除 Tailwind Play CDN + Google Fonts，
  按 AGENTS.md §3.3 编译离线 `static/static.css`（21KB 最小化）。成品零网络依赖。

### 可靠性修复
- **[高3] 锁不全**（`f41ab4c`）：`_lock` 只覆盖截图/OCR，cold_start/force_stop/reinstall 等
  11 个端点裸访问 `SESSION.device`。修复：新增 `Session.device_op()` 上下文管理器，
  8 个会触发 adb 的端点统一加锁。
- **[高4] adb 失败被吞**（`cdc6ccc`）：`check=False` 吞掉非零退出码，device offline 时
  uninstall 输出为空仍继续 install，覆盖安装被当干净重装。修复：空输出立即抛错。
- **[高5] 历史记录混算**（`8e1b3e5`）：自动报告 filter 所有 `source==='auto'` 记录，
  第二次运行混入第一次。修复：引入 `runStartIndex` + `run_id` 批次隔离。
- **[高6] 纯色模板误命中**（`e0d4ab7`）：TM_CCOEFF_NORMED 对纯色返回 1.0 满置信度，
...
### 自动停表优化
- **删除文字匹配模式**（`dbf48ac`）：实测 RapidOCR 全图推理 1373ms/次，精度 ±1-2s 不可接受。
  只保留模板比对（matchTemplate 3ms）。用户决策："1s 误差太大，直接放弃"。
- **设坐标不再真实点击设备**（`1302ffb`）：setCoords 去掉 POST tap，避免干扰设备。
- **卸装安装 log 加日期时间**（`8e8d2be`）：autoLog 从 `[HH:MM:SS]` → `[YYYY/MM/DD HH:MM:SS]`。

### 体验修复
- **循环时保持直播**（`08c924f`）：用户反馈"卸装重装看起来没生效"。根因是循环时停了直播，
...
### Git 项目管理
- `.gitattributes` 行尾规范化（`8a429b8`）
- pre-commit hook：防误提交 .venv + ast 语法检查（`7c7f6d0`，修正 `a9fa62d`）
- 项目更名为「App 冷启测速 / app-coldstart」（`080000e`）

---

## 2026-07-21 · 依赖修复 + 启动

- **补全 python-multipart 依赖**（`2a08a59`）：`/api/upload_apk` 用 UploadFile，
  FastAPI 要求 python-multipart，原 requirements.txt 漏写导致后端起不来。

---

## 2026-06 · v1 → v2 重写（项目初始化 `ef904f9`）
